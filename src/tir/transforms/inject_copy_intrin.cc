...
class CopyIntrinInjector : public StmtMutator {
 public:
  ...
  Stmt VisitStmt_(const ForNode* op) final {
    if (op->annotations.find(pragma_key_) != op->annotations.end()) {
      Stmt ret;
      std::string error_info;
      ICHECK(MatchCopyPattern(runtime::GetRef<Stmt>(op), &ret, &error_info))
          << "Cannot match copy pattern. The error is " << error_info << " The loop is "
          << op;
      return ret;
    }
    return StmtMutator::VisitStmt_(op);
