/*!
 * \file src/relax/backend/contrib/strela/codegen.cc
 * \brief STRELA JSON codegen implementation.
 */
...
class STRELAJSONSerializer : public JSONSerializer {
 public:
  STRELAJSONSerializer(ffi::Map<Constant, ffi::String> constant_names,
...
ffi::Array<ffi::Module> STRELACompiler(ffi::Array<Function> functions,
                                       ffi::Map<ffi::String, ffi::Any> /*unused*/,
                                       ffi::Map<Constant, ffi::String> constant_names) {
  ffi::Array<ffi::Module> compiled_functions;
  const auto pf = tvm::ffi::Function::GetGlobalRequired("runtime.STRELAJSONRuntimeCreate");

  for (const auto& func : functions) {
    STRELAJSONSerializer serializer(constant_names, AnalyzeVar2Value(func));
...
TVM_FFI_STATIC_INIT_BLOCK() {
  ...
  refl::GlobalDef().def("relax.ext.strela", STRELACompiler);
}
